...
void main() {
  ...
  testWidgets('converting a scratchpad to owed to me preserves its data', (
    tester,
  ) async {
    final repository = _RecordingEntryRepository();
    final entry = _entry(
    ...
    await _pumpForm(
      ...
      entry: entry,
      initialType: EntryType.scratchpad,
      conversionType: EntryType.owedToMe,
    );
    await tester.tap(find.widgetWithText(FilledButton, 'Save Changes'));
    await tester.pumpAndSettle();
    ...
    expect(saved.type, EntryType.owedToMe);
    expect(saved.title, entry.title);
    expect(saved.note, entry.note);
    expect(saved.amount, 75);
    expect(saved.createdAt, entry.createdAt);
    expect(saved.payments, hasLength(1));
    expect(saved.payments.single.note, 'Imported payment');
    expect(
      repository.entries.where((item) => item.type == EntryType.scratchpad),
      isEmpty,
    );
    expect(
      repository.entries.where((item) => item.type == EntryType.owedToMe),
      [saved],
    );
  });

  testWidgets('converting a scratchpad to owed by me keeps the same entry', (
    tester,
  ) async {
    final repository = _RecordingEntryRepository();
    final entry = _entry(type: EntryType.scratchpad, amount: 50);
    ...
    await _pumpForm(
      ...
      entry: entry,
      initialType: EntryType.scratchpad,
      conversionType: EntryType.owedByMe,
    );
    await tester.tap(find.widgetWithText(FilledButton, 'Save Changes'));
    await tester.pumpAndSettle();
    ...
    final saved = repository.saved!;
    expect(saved.id, entry.id);
    expect(saved.type, EntryType.owedByMe);
    expect(repository.entries, [saved]);
  });

  testWidgets('a scratchpad conversion requires a debt amount', (tester) async {
    final repository = _RecordingEntryRepository();
    final entry = _entry(type: EntryType.scratchpad, amount: null);
    ...
    expect(find.text('Amount is required for debt entries.'), findsOneWidget);
    expect(repository.saved, isNull);
    ...
    expect(repository.saved!.type, EntryType.owedToMe);
    expect(repository.saved!.amount, 25);
  });
  ...
  testWidgets('ordinary editing retains the existing entry type', (
    ...
    final repository = _RecordingEntryRepository();
    final scratchpad = _entry(type: EntryType.scratchpad, amount: null);
    ...
    await tester.tap(find.widgetWithText(FilledButton, 'Save Changes'));
    await tester.pumpAndSettle();
    expect(repository.saved!.type, EntryType.scratchpad);
